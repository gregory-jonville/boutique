<?php
    ...
    public function update_customer($id, $table, $fields = array())
  	{
  		$columns = implode(',', array_keys($fields));
  		$values  = ':'.implode(', :', array_keys($fields));
  		$sql = "UPDATE {$table} SET {$columns} = {$values} WHERE customer_id = '$id'";
  		$stmt = $this->pdo->prepare($sql);
    ...
    public function selectAllP()
    {
      $sql = "SELECT * FROM products ORDER BY add_date DESC";
      $stmt = $this->pdo->prepare($sql);
      $stmt->execute();
    ...
    public function selectAllProductByname($name) 
    {
      $sql = "SELECT * FROM products WHERE product_title = :product_title";
      $stmt = $this->pdo->prepare($sql);
       $stmt->bindParam(":product_title", $name);
      $stmt->execute();
    ...
    public function getRealUserIp()
    {
      ...
      $stmt = $this->pdo->prepare($sql);
      $stmt->bindParam(':customer_email', $customer);
      $stmt->execute();
      $customer = $stmt->fetch(PDO::FETCH_ASSOC);
      return $customer['customer_id'];
    }

    public function check_product_by_ip_id($id_cust, $p_id)
    {
      $sql = "SELECT * FROM cart WHERE id_cust = :id_cust AND p_id = :p_id";
      $stmt = $this->pdo->prepare($sql);
    ...
    public function count_product_by_ip($id_cust)
    {
      $sql = "SELECT * FROM cart WHERE id_cust = :id_cust";
      $stmt = $this->pdo->prepare($sql);
      $stmt->bindValue(":id_cust", $id_cust);
      $stmt->execute();
      $stmt->fetch();
      $count = $stmt->rowCount();
      return $count;
    ...
    public function select_products_by_ip($id_cust)
    {
      $sql = "SELECT * FROM cart WHERE id_cust = :id_cust";
      $stmt = $this->pdo->prepare($sql);
      $stmt->bindParam(":id_cust", $id_cust);
...
public function delete_p_cat($p_cat_id) {
  ...
  return $stmt->execute();
    ...
    public function delete_cart($id_cust)
    {
    	$sql = 	"DELETE FROM cart WHERE id_cust = :id_cust";
    	$stmt = $this->pdo->prepare($sql);
    	$stmt->bindParam(":id_cust", $id_cust);
    	return $stmt->execute();
    }
